...
public class RSAUtil {
 ...
 public static void generateKey() {
  ...
  try {
   ...
   // 进行转码
   publicKey = org.apache.tomcat.util.codec.binary.Base64.encodeBase64String(rsaPublicKey.getEncoded());
   // 进行转码
   privateKey = org.apache.tomcat.util.codec.binary.Base64.encodeBase64String(rsaPrivateKey.getEncoded());
 ...
 public static String encryptByprivateKey(String content, String privateKeyStr, int opmode) {
  // 私钥要用PKCS8进行处理
  PKCS8EncodedKeySpec pkcs8EncodedKeySpec = new PKCS8EncodedKeySpec(org.apache.tomcat.util.codec.binary.Base64.decodeBase64(privateKeyStr));
  ...
  try {
   ...
   if (opmode == Cipher.ENCRYPT_MODE) { // 加密
    ...
    text = org.apache.tomcat.util.codec.binary.Base64.encodeBase64String(result);
   } else if (opmode == Cipher.DECRYPT_MODE) { // 解密
    result = cipher.doFinal(org.apache.tomcat.util.codec.binary.Base64.decodeBase64(content));
 ...
 public static String encryptByPublicKey(String content, String publicKeyStr, int opmode) {
  // 公钥要用X509进行处理
  X509EncodedKeySpec x509EncodedKeySpec = new X509EncodedKeySpec(org.apache.tomcat.util.codec.binary.Base64.decodeBase64(publicKeyStr));
  KeyFactory keyFactory;
  PublicKey publicKey;
  Cipher cipher;
  byte[] result;
